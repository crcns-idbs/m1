    ...
    for nsession = 1:length(sessions)
        ...
        for nmovement = 1:length(movements)
            ...
            for nsegment = 1:length(segments)
                ...
                fname = ['segment_' num2str(nsegment) '_' session_name];
                ...
                if settings.write.stream.figures
                    myprint(fullfile(subject.raw_dir,'figures',['projection_' fname]))
                end
                ...
                title(strrep({[' N = ' num2str(numel(stream.raw.movement_onset)) ' '  subject.ID];[movement.channel]},'_',' '));
                ...
                if settings.write.stream.figures
                    myprint(fullfile(subject.raw_dir,'figures',['movement_' stream.fname]))
                end
                ...
                for c=1:numel(stream.settings.segments)
                    for d = 1:numel(stream.settings.freqnames)
                        ...
                        vfdata(c,:,d)=nanvar(fdata,[],2)'*stream.settings.projection_matrix;
                        if ~isempty(ecog.i)
                ...
                if settings.write.stream.figures
                    myprint(fullfile(subject.raw_dir,'figures',['stream_epoched_' stream.fname]))
                end
