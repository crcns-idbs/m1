...
switch config
    case 'default'
        ...
        settings.dir.template_root = fullfile(settings.dir.code_root(1:end-5),'templates');
        settings.dir.subject_root = 'E:\Dropbox (Brain Modulation Lab)\m1-master\subjects';
        fn =fieldnames(settings.dir);
        for a = 1:length(fn)
            if ~exist(settings.dir.(fn{a}),'dir')
                warning([fn{a} ' is not a folder!'])
            end
        end
        ...
        %% COMMON GRID ARCHITECTURE AND PROJECTION       
         ...
         settings.grid.ecog.max_distance = 25;
         ...
         settings.grid.lfp.stn.max_distance = 10;
